 ...
 */
export function createMcpServerResources(backend) {
    ...
    const mcpServerLogGroup = new logs.LogGroup(mcpServerStack, `${PROJECT_NAME}-mcp-server-logs`, {
        logGroupName: `/aws/lambda/${PROJECT_NAME}-mcp-server`,
        retention: logs.RetentionDays.ONE_WEEK,
        removalPolicy: RemovalPolicy.DESTROY,
    });
    // Create DynamoDB table for API key management
    const apiKeyTable = new dynamodb.Table(mcpServerStack, `${PROJECT_NAME}-api-keys`, {
    ...
    const lambdaRole = new iam.Role(mcpServerStack, `${PROJECT_NAME}-lambda-role`, {
    ...
    const mcpConstruct = new SimpleApiKeyMcpConstruct(mcpServerStack, `${PROJECT_NAME}-mcp-construct`, constructProps);
    // Get API URL for outputs
    const apiUrl = httpApi.url.replace(/\/$/, "");
