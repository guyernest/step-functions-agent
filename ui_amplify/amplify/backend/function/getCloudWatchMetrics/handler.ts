...
declare const process: { env: { AWS_REGION?: string; MODEL_COST_TABLE_NAME?: string } }
declare const require: any;

interface MetricRequest {
  metricType: 'cost' | 'tokens' | 'custom'
  startTime?: string
  endTime?: string
  period?: number
  agentName?: string
...
const DEFAULT_MODEL_PRICING: { [key: string]: { input: number; output: number } } = {
...
async function getModelCosts(): Promise<{ [key: string]: { input: number; output: number } }> {
  try {
    ...
    if (!modelCostTableName) {
      ...
      modelCostTableName = listTablesResponse.TableNames?.find((name: string) => 
        name.startsWith('ModelCost-')
      );
    }
    ...
    const modelPricing: { [key: string]: { input: number; output: number } } = { ...DEFAULT_MODEL_PRICING };
    
    if (scanResponse.Items) {
      scanResponse.Items.forEach((item: any) => {
...
export const handler = async (event: any): Promise<any> => {
  ...
  try {
    ...
    const request: MetricRequest = event.arguments || {}
    ...
    if (request.metricType === 'cost') {
      ...
      const idToMetricInfo: { [id: string]: { model: string; agent: string; isInput: boolean } } = {};
      let queryIndex = 0;

      // Process input metrics
      for (const metric of inputMetrics) {
        const modelDim = metric.Dimensions?.find((d: any) => d.Name === 'model');
        const agentDim = metric.Dimensions?.find((d: any) => d.Name === 'agent');
      ...
      const totalCostData: { [timestamp: string]: number } = {};
      const costByModel: { [model: string]: { [timestamp: string]: number } } = {};
      const costByAgent: { [agent: string]: { [timestamp: string]: number } } = {};
      const totalCostByModel: { [model: string]: number } = {};
      const totalCostByAgent: { [agent: string]: number } = {};
        ...
        for (let i = 0; i < metricQueries.length; i += batchSize) {
          ...
          if (response.MetricDataResults) {
            for (const result of response.MetricDataResults) {
              if (!result.Id || !result.Timestamps || !result.Values) continue;
              ...
              if (!costByModel[model]) costByModel[model] = {};
              if (!costByAgent[agent]) costByAgent[agent] = {};
              if (!totalCostByModel[model]) totalCostByModel[model] = 0;
              if (!totalCostByAgent[agent]) totalCostByAgent[agent] = 0;
              
              // Process each data point
              result.Timestamps.forEach((timestamp: Date, i: number) => {
      ...
      const costByModelFormatted: any = {};
      ...
      const costByAgentFormatted: any = {};
      ...
      const inputTokensByTime: { [timestamp: string]: number } = {};
      const outputTokensByTime: { [timestamp: string]: number } = {};
