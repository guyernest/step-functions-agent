...
async function executeToolTest(input) {
...
async function executeAgentTest(input) {
...
async function executeHealthCheck(input) {
...
async function discoverTestEvents(input) {
...
async function getTestEvent(resourceType, resourceId, testEventId) {
    const command = new GetItemCommand({
...
async function getTestEventsForResource(resourceType, resourceId) {
    const command = new QueryCommand({
        TableName: process.env.TEST_EVENTS_TABLE_NAME,
...
function convertDynamoDBMapToObject(map) {
    const result = {};
        ...
        if (value.S !== undefined) {
            ...
            result[key] = value.L.map((item) => {
                ...
                if (item.BOOL !== undefined)
                    return item.BOOL;
                if (item.M !== undefined)
                    return convertDynamoDBMapToObject(item.M);
                return item;
            });
        }
    }
