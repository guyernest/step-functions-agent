...
fn get_config() -> TransformConfig {
    ...
    let table_name = env::var("AGENT_CONTEXT_TABLE")
...
fn extract_record_id(reference: &str) -> Option<String> {
    if reference.starts_with(DYNAMODB_REF_PREFIX) {
        ...
        let id = &reference[DYNAMODB_REF_PREFIX.len()..];
        if !id.is_empty() {
    ...
    println!("[TRANSFORM] Failed to extract record ID from: {}", reference);
    None
}
...
pub async fn transform_input_event_async(event_body: &[u8]) -> Result<Vec<u8>, String> {
    ...
    let display_len = if event_str.len() > 500 { 500 } else { event_str.len() };
    println!("[TRANSFORM] Original event (truncated): {}...", &event_str[..display_len]);
    println!("[TRANSFORM] Original event length: {} bytes", event_body.len());
    ...
    match serde_json::from_slice::<Value>(event_body) {
        Ok(mut json_value) => {
            ...
            let result = serde_json::to_vec(&json_value).map_err(|e| {
                format!("Failed to serialize transformed event: {}", e)
            })?;
            
            println!("[TRANSFORM] Transformed event length: {} bytes", result.len());
            
            // Log a truncated version of the transformed event
            let transformed_str = String::from_utf8_lossy(&result);
            let display_len = if transformed_str.len() > 500 { 500 } else { transformed_str.len() };
            println!("[TRANSFORM] Transformed event (truncated): {}...", &transformed_str[..display_len]);
    ...
    match tokio::runtime::Handle::try_current() {
        Ok(handle) => {
            ...
            tokio::task::block_in_place(|| {
                handle.block_on(transform_input_event_async(event_body))
            })
        },
        Err(e) => {
...
async fn process_input_json(json: &mut Value, table_name: &str, max_depth: usize) {
    println!("[TRANSFORM] Processing input JSON to resolve DynamoDB references (max depth: {})", max_depth);
...
async fn process_input_json_by_levels(json: &mut Value, table_name: &str, max_depth: usize) {
    println!("[TRANSFORM] Processing input JSON by levels (max depth: {})", max_depth);
                            ...
                            if let Some(value2) = map1.get_mut(&key1) {
                                match value2 {
                                    Value::Object(ref mut map2) => {
                                        ...
                                        // Process level 3
                                        let level2_keys: Vec<String> = map2.keys().cloned().collect();
                                            ...
                                            if let Some(value3) = map2.get_mut(&key2) {
                                                match value3 {
                                                    Value::Object(ref mut map3) => {
                                                        check_content_for_reference(map3, table_name).await;
                                                        
                                                        // Process level 4
                                                        let level3_keys: Vec<String> = map3.keys().cloned().collect();
                                                        for key3 in level3_keys {
                                                            if max_depth < 4 { continue; }
                                                            
                                                            if let Some(value4) = map3.get_mut(&key3) {
                                                                match value4 {
                                                                    Value::Object(ref mut map4) => {
                                                                        check_content_for_reference(map4, table_name).await;
                                                                        
                                                                        // Process level 5
                                                                        let level4_keys: Vec<String> = map4.keys().cloned().collect();
                                                                        for key4 in level4_keys {
                                                                            if max_depth < 5 { continue; }
                                                                            
                                                                            if let Some(value5) = map4.get_mut(&key4) {
                                                                                match value5 {
                                                                    ...
                                                                    Value::Array(ref mut items4) => {
                                                                        for item4 in items4.iter_mut() {
                                                                            if let Value::Object(ref mut item_map4) = item4 {
                                                    ...
                                                    Value::Array(ref mut items3) => {
                                                        for item3 in items3.iter_mut() {
                                                            if let Value::Object(ref mut item_map3) = item3 {
                                                                check_content_for_reference(item_map3, table_name).await;
                                                            }
                                                        }
                                                    },
                                                    _ => {}
                                    ...
                                    Value::Array(ref mut items2) => {
                                        for item2 in items2.iter_mut() {
                                            if let Value::Object(ref mut item_map2) = item2 {
                                                check_content_for_reference(item_map2, table_name).await;
                                            }
                                        }
                                    },
                                    _ => {}
                                        ...
                                        match item1_value {
                                            ...
                                            Value::Array(ref mut inner_items) => {
                                                for inner_item in inner_items.iter_mut() {
                                                    if let Value::Object(ref mut inner_item_map) = inner_item {
                                                        check_content_for_reference(inner_item_map, table_name).await;
                                                    }
                                                }
                                            },
                                            _ => {}
            ...
            if let Value::Object(ref mut item_map) = item {
                ...
                if max_depth >= 1 {
                    ...
                    for item_key in item_keys {
                        if let Some(item_value) = item_map.get_mut(&item_key) {
                            if let Value::Object(ref mut inner_map) = item_value {
                                check_content_for_reference(inner_map, table_name).await;
                            }
                        }
                    }
    ...
    println!("[TRANSFORM] Completed processing up to {} levels deep", max_depth);
}
...
pub async fn transform_response_async(response_body: &[u8]) -> Result<Vec<u8>, String> {
    ...
    let display_len = if response_str.len() > 500 { 500 } else { response_str.len() };
    println!("[TRANSFORM] Original response (truncated): {}...", &response_str[..display_len]);
    println!("[TRANSFORM] Original response length: {} bytes", response_body.len());
    ...
    match serde_json::from_slice::<Value>(response_body) {
        Ok(mut json_value) => {
            // Get configuration
            let config = get_config();
            println!("[TRANSFORM] Using max_content_size: {}, max_json_depth: {}", 
            ...
            process_output_json(&mut json_value, &config.table_name, config.max_content_size, config.max_json_depth).await;
            ...
            // Serialize back to JSON
            let result = serde_json::to_vec(&json_value).map_err(|e| {
                format!("Failed to serialize transformed response: {}", e)
            })?;
            
            println!("[TRANSFORM] Transformed response length: {} bytes", result.len());
            
            // Log a truncated version of the transformed response
            let transformed_str = String::from_utf8_lossy(&result);
            let display_len = if transformed_str.len() > 500 { 500 } else { transformed_str.len() };
            println!("[TRANSFORM] Transformed response (truncated): {}...", &transformed_str[..display_len]);
...
pub fn transform_response(response_body: &[u8]) -> Result<Vec<u8>, String> {
    ...
    match tokio::runtime::Handle::try_current() {
        Ok(handle) => {
            // We're in a tokio runtime, use block_in_place to prevent blocking the runtime
            tokio::task::block_in_place(|| {
                handle.block_on(transform_response_async(response_body))
            })
        },
        Err(e) => {
            println!("[TRANSFORM] Not running in tokio runtime: {}", e);
...
async fn process_output_json(json: &mut Value, table_name: &str, max_content_size: usize, max_depth: usize) {
    println!("[TRANSFORM] Processing output JSON to store large content (max depth: {})", max_depth);
    ...
    if let Value::Object(ref mut map) = json {
        ...
        if let Some(Value::String(body_str)) = map.get("body").cloned() {
            if let Ok(mut body_json) = serde_json::from_str::<Value>(&body_str) {
                ...
                process_output_json_by_levels(&mut body_json, table_name, max_content_size, max_depth).await;
...
async fn process_output_json_by_levels(json: &mut Value, table_name: &str, max_content_size: usize, max_depth: usize) {
    println!("[TRANSFORM] Processing output JSON by levels (max depth: {})", max_depth);
                    ...
                    Value::Object(ref mut map1) => {
                        ...
                        for key1 in level1_keys {
                            ...
                            if let Some(value2) = map1.get_mut(&key1) {
                                match value2 {
                                    Value::Object(ref mut map2) => {
                                        check_content_size(map2, table_name, max_content_size).await;
                                        ...
                                        for key2 in level2_keys {
                                            ...
                                            if let Some(value3) = map2.get_mut(&key2) {
                                                match value3 {
                                                    Value::Object(ref mut map3) => {
                                                        check_content_size(map3, table_name, max_content_size).await;
                                                        ...
                                                        for key3 in level3_keys {
                                                            ...
                                                            if let Some(value4) = map3.get_mut(&key3) {
                                                                match value4 {
                                                                    Value::Object(ref mut map4) => {
                                                                        check_content_size(map4, table_name, max_content_size).await;
                                                    ...
                                                    Value::Array(ref mut items3) => {
                                                        for item3 in items3.iter_mut() {
                                                            if let Value::Object(ref mut item_map3) = item3 {
                                                                check_content_size(item_map3, table_name, max_content_size).await;
                                                            }
                                                        }
                                                    },
                                                    _ => {}
                                    ...
                                    Value::Array(ref mut items2) => {
                                        for item2 in items2.iter_mut() {
                                            if let Value::Object(ref mut item_map2) = item2 {
                                                check_content_size(item_map2, table_name, max_content_size).await;
                                            }
                                        }
                                    },
                                    _ => {}
                            ...
                            if let Value::Object(ref mut item_map1) = item1 {
                                ...
                                for item1_key in item1_keys {
                                    if let Some(item1_value) = item_map1.get_mut(&item1_key) {
                                        match item1_value {
                                            Value::Object(ref mut inner_map) => {
                                                check_content_size(inner_map, table_name, max_content_size).await;
                                            },
                                            Value::Array(ref mut inner_items) => {
                                                for inner_item in inner_items.iter_mut() {
                                                    if let Value::Object(ref mut inner_item_map) = inner_item {
                                                        check_content_size(inner_item_map, table_name, max_content_size).await;
                                                    }
                                                }
                                            },
                                            _ => {}
    ...
    println!("[TRANSFORM] Completed processing output for content size up to {} levels deep", max_depth);
}

/// Check if a JSON object has a "content" field that exceeds the size threshold and store it in DynamoDB
async fn check_content_size(map: &mut Map<String, Value>, table_name: &str, max_content_size: usize) {
    if let Some(Value::String(content_str)) = map.get("content") {
        ...
        if content_len > max_content_size {
            ...
            match dynamodb_put_item(table_name, &content_value).await {
                Ok(record_id) => {
                    ...
                    println!("[TRANSFORM] Replaced large content with DynamoDB reference: {}", dynamo_ref);
                },
                Err(error) => {
...
async fn dynamodb_get_item(table_name: &str, record_id: &str) -> Result<Value, String> {
    ...
    let request = client.get_item()
        ...
        .key("id", aws_sdk_dynamodb::types::AttributeValue::S(record_id.to_string()));
            ...
            if let Some(item) = response.item {
                ...
                if let Some(content_attr) = item.get("content") {
                    if let aws_sdk_dynamodb::types::AttributeValue::S(content_str) = content_attr {
                        ...
                        Err(format!("Content is not a string in DynamoDB record {}", record_id))
                    }
                } else {
                    Err(format!("Content attribute not found in DynamoDB record {}", record_id))
                }
            } else {
                Err(format!("Record {} not found in DynamoDB table {}", record_id, table_name))
            }
        },
...
async fn dynamodb_put_item(table_name: &str, content: &Value) -> Result<String, String> {
    ...
    let request = client.put_item()
        .table_name(table_name)
        .item("id", aws_sdk_dynamodb::types::AttributeValue::S(record_id.clone()))
        .item("content", aws_sdk_dynamodb::types::AttributeValue::S(content_json))
        .item("timestamp", aws_sdk_dynamodb::types::AttributeValue::N(Utc::now().timestamp().to_string()));
    
    // Execute the request
    match request.send().await {
        Ok(_) => {
            println!("[TRANSFORM] Stored {} bytes in DynamoDB table {} with ID {}", 
