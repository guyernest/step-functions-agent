...
class ActionExecutor:
    ...
    def _get_position(self, action: Dict) -> tuple:
        ...
        if "position" in action:
            ...
            return self._locate_image(action["image"], action.get("confidence", 0.9))
        else:
    ...
    def _locate_image(self, image_path: str, confidence: float = 0.9) -> tuple:
        ...
        try:
            # First try the standard PyAutoGUI approach
            location = self.pyautogui.locateCenterOnScreen(image_path, confidence=confidence)
            if location is not None:
                logger.info(f"Found image {image_path} at position {location} using PyAutoGUI")
                return location
            ...
            if not hasattr(self, 'cv2'):
                ...
                screenshot_path = self._capture_debug_screenshot(f"image_not_found_{os.path.basename(image_path)}")
                raise ValueError(f"Could not locate image: {image_path}")
            ...
            try:
                ...
                template = self.cv2.imread(image_path)
                if template is None:
                    raise ValueError(f"Could not load image file: {image_path}")
                
                # Convert images to grayscale for template matching
                gray_screenshot = self.cv2.cvtColor(screenshot_cv, self.cv2.COLOR_BGR2GRAY)
                gray_template = self.cv2.cvtColor(template, self.cv2.COLOR_BGR2GRAY)
                
                # Perform template matching
                result = self.cv2.matchTemplate(gray_screenshot, gray_template, self.cv2.TM_CCOEFF_NORMED)
                ...
                if max_val >= confidence:
                    ...
                    logger.info(f"Found image {image_path} at position ({center_x}, {center_y}) using OpenCV with confidence {max_val:.2f}")
                    return (center_x, center_y)
                
                # If we got here, neither method found the image
                # Capture screen for debugging
                screenshot_path = self._capture_debug_screenshot(f"low_confidence_{os.path.basename(image_path)}")
                
                # Draw the best match on the screenshot for debugging
                if max_loc and template is not None:
                    debug_img = screenshot_cv.copy()
                    h, w = template.shape[:2]
                    self.cv2.rectangle(debug_img, max_loc, (max_loc[0] + w, max_loc[1] + h), (0, 0, 255), 2)
                    ...
                    logger.info(f"Saved annotated debug image to {debug_path}")
                
                raise ValueError(f"Could not locate image: {image_path} (best OpenCV match: {max_val:.2f})")
            except Exception as cv_error:
                if not screenshot_path:
                    screenshot_path = self._capture_debug_screenshot(f"opencv_error_{os.path.basename(image_path)}")
                logger.error(f"OpenCV image matching failed: {cv_error}")
                raise ValueError(f"Could not locate image: {image_path} (OpenCV error: {cv_error})")
                
        except Exception as e:
            if not screenshot_path:
                screenshot_path = self._capture_debug_screenshot(f"error_{os.path.basename(image_path)}")
            logger.error(f"Error trying to locate image {image_path}: {e}")
            raise ValueError(f"Could not locate image: {image_path} due to error: {e}", screenshot_path)
    ...
    def _handle_locate_image(self, action: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "position": (x, y), "image": image_path}
...
def main():
    ...
    if args.script.endswith('.json') or '/' in args.script:
        try:
